  ...
  rendition.hooks.content.register((contents) => {
    ...
    function detectCoverPage() {
      try {
        ...
        console.log('🔍 Page analysis - Images:', images.length, 'Text length:', totalTextLength);
        ...
        if (isCover) {
          ...
          console.log('📕 ✅ COVER PAGE DETECTED! Href:', href, 'Images:', images.length, 'Text:', totalTextLength);
          ...
          console.log('📄 Normal content page. Href:', href);
        }
      } catch (e) {
        console.log('Error in detectCoverPage:', e);
      }
    }
...
function setFontSize(fontSize) {
  ...
  console.log('Current theme - bg:', bgColor, 'fg:', fgColor, 'css:', css);
  
  // Add font-size to customCss
  css['font-size'] = fontSize + 'px';
  console.log('Updated css with font-size:', css);
  
  // Reapply theme with new font size
  updateTheme(bgColor, fgColor, css);
  rendition.reportLocation();
  console.log('=== setFontSize COMPLETE ===');
}

//get page information (current page and total pages)
function getPageInfo() {
  console.log('getPageInfo called');
  console.log('book.locations:', book.locations);
  
  if (!book.locations) {
    console.log('No locations available, returning 1/1');
    return { currentPage: 1, totalPages: 1 };
  }
  
  // Call length() as a function, not as a property
  var totalPages = book.locations.length();
  console.log('book.locations.length():', totalPages);
  
  if (!totalPages || totalPages === 0) {
    console.log('Total pages is 0 or null, returning 1/1');
    return { currentPage: 1, totalPages: 1 };
  }
  ...
  if (rendition && rendition.location && rendition.location.start) {
    ...
    if (currentCfi) {
      try {
        ...
        console.log('Location index from CFI:', locationIndex);
        
        if (locationIndex && locationIndex > 0) {
          currentPage = locationIndex;
        }
      } catch (e) {
        console.log('Error getting page from CFI:', e);
      }
    }
  }
  
  console.log('Returning currentPage:', currentPage, 'totalPages:', totalPages);
  return { currentPage: currentPage, totalPages: totalPages };
}

//get page number from CFI or href
function getPageFromCfi(cfiOrHref) {
  console.log('getPageFromCfi called with:', cfiOrHref);
  ...
  var totalPages = book.locations.length();
  if (!totalPages || totalPages === 0) {
    console.log('Total pages is 0');
    return null;
  }
  
  try {
    // If it's just a href (like "chapter1.xhtml"), convert it to CFI
    var cfi = cfiOrHref;
    if (!cfiOrHref.startsWith('epubcfi(')) {
      // It's an href, find the spine item
      var spineItem = book.spine.get(cfiOrHref);
      if (spineItem) {
        // Create a CFI pointing to the start of this spine item
        cfi = spineItem.cfiBase;
        console.log('Converted href to CFI:', cfi);
      }
    }
    
    var locationIndex = book.locations.locationFromCfi(cfi);
    console.log('Location index for', cfiOrHref, ':', locationIndex);
...
function updateTheme(backgroundColor, foregroundColor, customCss) {
  ...
  console.log('customCss type:', typeof customCss);
  console.log('customCss value:', customCss);
  console.log('customCss keys:', customCss ? Object.keys(customCss) : 'null');
  ...
  if (backgroundColor && backgroundColor !== "" && backgroundColor !== "null") {
    ...
    console.log('✓ Background color added:', backgroundColor);
  }
  if (foregroundColor && foregroundColor !== "" && foregroundColor !== "null") {
    bodyStyles["color"] = foregroundColor;
    console.log('✓ Foreground color added:', foregroundColor);
  }
  ...
  if (customCss && typeof customCss === 'object') {
    if (customCss['font-family']) {
      ...
      console.log('✓ Font-family added to body:', customCss['font-family']);
    }
    if (customCss['font-weight']) {
      bodyStyles["font-weight"] = customCss['font-weight'] + " !important";
      console.log('✓ Font-weight added to body:', customCss['font-weight']);
    }
    if (customCss['font-size']) {
      bodyStyles["font-size"] = customCss['font-size'] + " !important";
      console.log('✓ Font-size added to body:', customCss['font-size']);
    }
  }
  ...
  if (Object.keys(textStyles).length > 0) {
    ...
    console.log('✓ Text element styles:', textStyles);
  }
  ...
  if (Object.keys(rules).length > 0) {
    ...
    console.log('✓ Theme registered and selected: user-theme');
  } else {
    console.log('❌ No rules to apply!');
  }
  console.log('=== updateTheme COMPLETE ===');
}
